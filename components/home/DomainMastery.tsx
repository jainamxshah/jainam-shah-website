      ...
      <div className="max-w-[1400px] mx-auto px-6 md:px-8 lg:px-12">
        <div className="flex flex-col lg:flex-row items-center lg:items-center gap-8 lg:gap-12">
          ...
          <div className="absolute inset-0 flex items-center justify-center z-20 pointer-events-none">
            ...
            >
              <div 
                className="w-20 h-20 md:w-24 md:h-24 lg:w-28 lg:h-28 bg-accent border-2 border-accent flex items-center justify-center group hover:scale-105 transition-transform duration-300"
                style={{
                  clipPath: 'polygon(50% 0%, 100% 25%, 100% 75%, 50% 100%, 0% 75%, 0% 25%)'
                }}
              >
                <span className="font-kalice text-lg md:text-xl lg:text-2xl text-foreground font-bold">JS</span>
              </div>
            </motion.div>
          </div>
  ...
  return (
    ...
    >
        <motion.div
          ...
          className="bg-foreground border border-background/20 rounded-xl p-3 md:p-4 lg:p-5 w-[200px] md:w-[260px] lg:w-[300px] shadow-sm hover:shadow-md hover:border-accent/50 transition-all duration-300"
        >
          <h3 className="font-neue text-xs md:text-sm lg:text-base font-semibold text-background mb-1 md:mb-1.5">
            {domain.name}
          </h3>
          <p
            className={`font-neue text-[10px] md:text-xs lg:text-sm text-background/70 leading-[1.5] transition-opacity duration-300 ${
              isHovered ? 'opacity-100' : 'opacity-80 md:opacity-70'
            }`}
          >
            {domain.description}
          </p>
