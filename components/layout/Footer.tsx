    ...
    <footer className="bg-foreground text-background">
      <div className="container-main py-16 md:py-20">
        ...
        <div className="flex flex-col md:flex-row md:justify-between md:items-start gap-12 md:gap-8">
          {/* Left Side - Brand */}
          <div>
            <Link href="/" className="text-xl font-medium text-background hover:text-accent transition-colors">
              Jainam Shah
            </Link>
            <p className="text-sm text-background/70 mt-3 max-w-xs">
              Building AI-powered products that win.
            </p>
          </div>
          ...
          <nav className="flex flex-col items-start md:items-end gap-3">
            {footerLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="text-sm text-background/80 hover:text-accent transition-colors duration-300 relative group"
              >
                {link.label}
